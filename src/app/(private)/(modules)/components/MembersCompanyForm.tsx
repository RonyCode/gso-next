...
export const MembersCompanyForm = ({
  corporation,
  company,
  className,
  ...
  const handleSubmit = (formData: Partial<IUnidadeSchema>): void => {
    startTransition(async () => {
      ...
      if (result?.code === 202) {
        ...
        redirect(`/servicos/unidades/${company?.name}-${company?.id}/membros`)
      }
    })
  }
  const $memberWithoutComp = corporation?.members?.filter((memberItem) => {
    return (
      memberItem?.id !== null &&
      memberItem?.id !==
                    ...
                    render={({ field }) => (
                      <FormItem className="flex w-full flex-col">
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-[200px] p-0">
                            <Command>
                              <CommandInput placeholder="Procurando Membros..." />
                              <CommandEmpty>
                                Membro não encontrada.
                              </CommandEmpty>
                              <CommandGroup>
                                <CommandList>
                                  {$memberWithoutComp?.map((membro, index) => (
                                    <CommandItem
                                      value={membro?.id ?? undefined}
                                      key={index}
                                      /* eslint-disable-next-line @typescript-eslint/no-misused-promises */
                                      onSelect={async () => {
                                        form.setValue('id_member', membro?.id)
                                      }}
                                    >
                                      <LuCheck
                                        className={cn(
...
export default MembersCompanyForm
